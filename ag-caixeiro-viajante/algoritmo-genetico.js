...
function selecaoTorneio(populacao, distancias, tamanhoTorneio) {
  let indiceMelhor = inteiroAleatorio(populacao.length);
  for (let competidor = 1; competidor < tamanhoTorneio; competidor++) {
    const indiceSorteado = inteiroAleatorio(populacao.length);
    if (distancias[indiceSorteado] < distancias[indiceMelhor])
      indiceMelhor = indiceSorteado; // menor distância = melhor
  }
  return populacao[indiceMelhor];
}
...
function cidadesLivres(destino) {
  ...
  for (let cidade = 0; cidade < NUM_CIDADES; cidade++) {
    ...
    if (destino !== ORIGEM && cidade === destino) continue; // destino é fixo no fim
    livres.push(cidade);
  }
  return livres;
}
...
function rodarAG(config, aoAvancarGeracao, aoFinalizar) {
  ...
  function passo() {
    ...
    // ordena índices por distância (crescente = melhores primeiro) para o elitismo
    const indicesOrdenados = populacao
      .map((_, indice) => indice)
      .sort((a, b) => distancias[a] - distancias[b]);

    const proximaPopulacao = [];
    // elitismo: copia os melhores diretamente para a próxima geração
    ...
    while (proximaPopulacao.length < config.populacao) {
      const pai1 = selecaoTorneio(populacao, distancias, config.torneio);
      const pai2 = selecaoTorneio(populacao, distancias, config.torneio);
      let filho =
...
async function desenharRotaEstrada(rota) {
  ...
  try {
    ...
    if (dados.code !== "Ok" || !dados.routes.length) throw new Error(dados.code);
...
elemento("run").addEventListener("click", () => {
  ...
  const config = {
    ...
    torneio: Number(elemento("torn").value),
